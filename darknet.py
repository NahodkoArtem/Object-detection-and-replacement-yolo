        ...
        try:
            ...
            for detection in detections:
                if detection[0] == "aeroplane":
                    ...
                    print(xCoord, yCoord, xCoord + xEntent, yCoord + yExtent)
                    
                    imagez = cv2.imread(imagePath)
                    image_rectangled = cv2.rectangle(imagez, (xCoord, yCoord), (xCoord + xEntent, yCoord + yExtent), (0, 0, 0), -1)
                    cv2.imwrite("output.png", image_rectangled)

                    image = cv2.imread(imagePath)
                    mask = cv2.imread("output.png", 0)
                    mask = cv2.medianBlur(mask,5)
                    ret,th1 = cv2.threshold(mask,127,255,cv2.THRESH_BINARY_INV)
                    output = cv2.inpaint(image, th1, 3, cv2.INPAINT_TELEA)
                    cv2.imwrite("lol.jpg", output)

                    # Read images
                    dst = cv2.imread("lol.jpg")
                    ...
                    # Clone seamlessly.
                    if dst.shape[0] < bounds[1] + src.shape[0]:
                        y = dst.shape[0] - src.shape[0] - 10
                    else:
                        y = bounds[1]

                    if dst.shape[1] < bounds[0] + src.shape[1]:
                        x = dst.shape[1] - src.shape[1] - 10
                    else:
                        x = bounds[0]
                    print(x, y)
                    print(src.shape, dst.shape)
                    output = cv2.seamlessClone(src, dst, src_mask, (720, 300), cv2.NORMAL_CLONE)

                    # Save result
                    cv2.imwrite("kkk.jpg", output)

                    im1 = Image.open('kkk.jpg')
                    im1.show()
